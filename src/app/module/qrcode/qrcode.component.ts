...
export class QRCodeComponent implements OnChanges {
  ...
  @Input() public qrData: QRCodeSegment[] = [];
  ...
  protected isValidQrCodeText(data: string | QRCodeSegment[] | null): boolean {
  ...
  private toDataURL(qrCodeConfig: QRCodeToDataURLOptions): Promise<FixMeLater> {
    return new Promise((resolve: (arg: FixMeLater) => FixMeLater, reject: (arg: FixMeLater) => FixMeLater) => {
  ...
  private toCanvas(canvas: HTMLCanvasElement, qrCodeConfig: QRCodeRenderersOptions): Promise<FixMeLater> {
    return new Promise((resolve: (arg: FixMeLater) => FixMeLater, reject: (arg: FixMeLater) => FixMeLater) => {
  ...
  private async createQRCode(): Promise<void> {
    ...
    if (this.version && this.version > 40) {
      console.warn('[yimiao-qrcode] max value for `version` is 40');
      this.version = 40;
    } else if (this.version && this.version < 1) {
      console.warn('[yimiao-qrcode]`min value for `version` is 1');
      this.version = 1;
    } else if (this.version !== undefined && isNaN(this.version)) {
      console.warn('[yimiao-qrcode] version should be a number, defaulting to auto.');
      this.version = undefined;
    }

    try {
      if (!this.isValidQrCodeText(this.qrData)) {
        throw new Error('[yimiao-qrcode] Field `qrdata` is empty, set \'allowEmptyString="true"\' to overwrite this behaviour.');
      }

      // This is a workaround to allow an empty string as qrdata
      if (this.isValidQrCodeText(this.qrData) && this.qrData.length === 0) {
        this.qrData = [];
      }
      ...
      switch (this.elementType) {
        case 'canvas': {
          ...
          this.toCanvas(canvasElement, config)
            ...
            .catch(e => {
              console.error('[yimiao-qrcode] canvas error:', e);
            });
          break;
        }
        case 'svg': {
          ...
          this.toSVG(config)
            ...
            .catch(e => {
              console.error('[yimiao-qrcode] svg error:', e);
            });
          break;
        }
        case 'url':
        ...
        default: {
          ...
          this.toDataURL(config)
            ...
            .catch(e => {
              console.error('[yimiao-qrcode] img/url error:', e);
            });
        }
      }
    } catch (e: FixMeLater) {
      console.error('[yimiao-qrcode] Error generating QR Code:', e.message);
    }
  }
  ...
  emitQRCodeURL(element: HTMLCanvasElement | HTMLImageElement | SVGSVGElement) {
    ...
    fetch(urlImage)
      ...
      .catch(error => {
        console.error(`[yimiao-qrcode] Error when fetching image/png URL: ${error}`);
      });
  }
}
